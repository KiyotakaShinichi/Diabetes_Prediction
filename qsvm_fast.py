# qsvm_model_fast.py
# Quadratic SVM classification pipeline (no GridSearchCV)
# Includes: CV accuracies, learning curve, confusion matrix, ROC curve, PCA visualization, and full metrics report.
...
experiment_config.require_direct_execution(__name__, 'qsvm_fast.py')
_ARGS = experiment_config.parse_args(
    'qsvm_fast.py',
    default_data_path=experiment_config.LEGACY_DATA_PATH,
)
DATA_PATH = _ARGS.data_path
FINAL_CSV = experiment_config.result_path(_ARGS, 'final_results_qsvm_fast.csv')
...
# ---------------------------
# 3) Quadratic SVM (Direct)
# ---------------------------
print("\n⚙️ Training Quadratic SVM (no GridSearch)...")
svm_best = SVC(
    kernel='poly',
    degree=2,
    C=1.0,
