...
# Includes: GridSearchCV, CV per-fold accuracies, learning curve, confusion matrix,
...
experiment_config.require_direct_execution(__name__, 'xgboost_only.py')
_ARGS = experiment_config.parse_args(
    'xgboost_only.py',
    default_data_path=experiment_config.LEGACY_DATA_PATH,
)
DATA_PATH = _ARGS.data_path
FINAL_CSV = experiment_config.result_path(_ARGS, 'final_results_xgboost.csv')
...
for train_ix, val_ix in skf.split(X_train, y_train):
    model = XGBClassifier(**xgb_best.get_params())
...
fig_folds.update_layout(title="Cross-validation fold accuracies (XGBoost)", yaxis_title="Accuracy")
fig_folds.show()
...
fig_lc.update_layout(title='Learning Curve - XGBoost', xaxis_title='Number of training examples', yaxis_title='Accuracy')
fig_lc.show()
...
cm_fig.update_layout(title='Confusion Matrix - XGBoost (Test set)')
cm_fig.show()

# ROC curve
fpr, tpr, _ = roc_curve(y_test, proba_test)
roc_fig = go.Figure()
roc_fig.add_trace(go.Scatter(x=fpr, y=tpr, mode='lines', name=f'XGBoost (AUC={auc:.3f})'))
roc_fig.add_trace(go.Scatter(x=[0,1], y=[0,1], mode='lines', line=dict(dash='dash'), name='Random'))
roc_fig.update_layout(title='ROC Curve - XGBoost (Test set)', xaxis_title='FPR', yaxis_title='TPR')
roc_fig.show()

# ---------------------------
# 7) PCA Visualization (Test)
# ---------------------------
...
fig_pca = px.scatter(
    ...
    title='PCA Projection - XGBoost (Test set)'
)
fig_pca.show()
